...
static int alloc_dma_bufs(struct device *dev, struct mlb_dev_info *pdevinfo, enum channelmode mode)
{
	...
	if (drvdata->sram_pool) {
		pdevinfo->dma_cpu = gen_pool_dma_alloc(drvdata->sram_pool,
					dma_size, &pdevinfo->dma_handle);

		if (likely(pdevinfo->dma_cpu))
			memset(pdevinfo->dma_cpu, 0x00, dma_size);
	} else
...
static void free_dma_bufs(struct device *dev, struct mlb_dev_info *pdevinfo)
{
	...
	if (drvdata->sram_pool)
		gen_pool_free(drvdata->sram_pool,
			(unsigned long)pdevinfo->dma_cpu, pdevinfo->dma_size);
	else
...
struct mlb150_sram_gen_pool_cb_info {
	struct platform_device *pdev;
	int n;
};

static void mlb150_sram_gen_pool_cb(struct gen_pool *pool,
	struct gen_pool_chunk *chunk, void *data)
{
	struct mlb150_sram_gen_pool_cb_info *info = data;
	unsigned long chunk_size = chunk->end_addr - chunk->start_addr + 1;

	dev_info(&info->pdev->dev, " SRAM region %i: %ld KiB @ 0x%llx",
		info->n, chunk_size / 1024, chunk->phys_addr);
...
static int mlb150_probe(struct platform_device *pdev)
{
	...
	/*
	 * Get the SRAM pool allocator from the device tree.
	 * If not defined, the regular DMA allocator will be used.
	 */
	pool = of_get_named_gen_pool(pdev->dev.of_node, "sram_dma_area", 0);
	if (pool) {
		struct mlb150_sram_gen_pool_cb_info cb_info = {
			.pdev = pdev,
			.n = 1,
		};

		dev_info(&pdev->dev, "DMA buffers will be allocated from SRAM:");
		gen_pool_for_each_chunk(pool, mlb150_sram_gen_pool_cb,
					&cb_info);
		drvdata->sram_pool = pool;
	}
