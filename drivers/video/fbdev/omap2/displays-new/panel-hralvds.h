/*********************************************************
*
*  Copyright (C) 2014 - 2019 Honda R&D Americas, Inc.
...
/*
 * The following is a list of display quirks or bugs.
 */
#define HRALVDS_PANEL_QUIRKS_NONE			(0)
#define HRALVDS_PANEL_QUIRKS_PANASONIC_DISPC_BUG	(1 << 0)

enum {
	HRALVDS_PANEL_TYPE_THRA = 0,
	HRALVDS_PANEL_TYPE_TJBA,
	HRALVDS_PANEL_TYPE_MAX
};
...
/*
 * The version command reply is common to all HRA LVDS displays (for now)
 */
union hralvds_version_cmd_reply_item {
	u16 version;
	struct {
		u8 ver_major;
		u8 ver_minor;
...
struct hralvds_version_cmd_reply_struct {
	union {
		u8 data[HRALVDS_VERSION_RSP_SIZE];
		struct {
			union hralvds_version_cmd_reply_item hw;
			union hralvds_version_cmd_reply_item sw;
			union hralvds_version_cmd_reply_item touch_ic;
			u8 option;
...
typedef enum {
	...
	POWER_STATE_POWERING_OFF_GCF,	/* GCF occurred, DISP_CONT still 1 */
	POWER_STATE_POWERING_OFF_CHECK_ON, /* check if user wants ON */
	POWER_STATE_POWERING_ON_0,  /* DISPC_CONT on, link not established */
...
struct panel_drv_data {
	...
	int panel_type;		/* THRA or TJBA */
	unsigned long panel_quirks;

	/* log ratelimit state for setup_link, to prevent log spam */
	struct ratelimit_state setup_link_rs;

	/* i2c command logging rate limit */
	struct ratelimit_state cmd_rs;
	...
	/*
	 * The following are the timestamp in jiffies of the last
	 * DISP_CONT toggle.
	 */
	unsigned long dispc_on_off_jiffies;
	unsigned long dispc_off_on_jiffies;

	unsigned long ack_timeout_jiffies;
};
...
int hralvds_get_panel_type(void);

unsigned long hralvds_get_default_panel_quirks(void);
